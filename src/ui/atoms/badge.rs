...
pub fn count_pill(ui: &mut Ui, count: &str) {
    let (bg, fg) = if ui.visuals().dark_mode {
        (Color32::from_rgb(0x22, 0x2a, 0x36), Color32::from_rgb(0x99, 0xa3, 0xb2))
    } else {
        (Color32::from_rgb(0xe6, 0xe9, 0xee), Color32::from_rgb(0x59, 0x62, 0x6f))
    };
...
fn badge_pill(ui: &mut Ui, text: &str, bg: Color32, fg: Color32) {
    ...
    if ui.is_rect_visible(rect) {
        ui.painter().rect_filled(rect, egui::CornerRadius::same(255u8), bg);
